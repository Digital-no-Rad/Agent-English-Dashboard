...
st.title("🧑‍🏫 Agent English AI - Teacher Dashboard")
...
tab1, tab2, tab3 = st.tabs(["🎤 Voice Grader", "📝 Homework Grader", "🍎 Lesson Maker"])
...
if api_key_content:
    try:
        ...
        st.success("✅ Connected to Google Cloud! All tools are active.")
        
    except DefaultCredentialsError:
        st.error("❌ Authentication Error: The key provided is invalid. Please ensure the key content is correct.")
        st.stop()
    except Exception as e:
        if 'key_path' in locals() and os.path.exists(key_path):
            os.remove(key_path)
        st.error(f"❌ Invalid Key or Initialization Error: {e}")
        st.stop()
    
    # --- TAB 1: VOICE GRADER (SMART AUTO-DETECT) ---
    with tab1:
        st.header("🎤 Voice Grader")
    ...
    with tab2:
        st.header("📝 Homework Grader (Powered by Gemini)")
        st.write("Upload a student's handwritten homework (PNG or JPG).")
        
        homework_file = st.file_uploader("Upload image", type=['png', 'jpg', 'jpeg'], key="homework_uploader")
    ...
    with tab3:
        st.header("🍎 Lesson Maker (Powered by Gemini)")
